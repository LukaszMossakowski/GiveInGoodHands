    ...
    def delete_selected(self, request, queryset):
        for obj in queryset:
            if obj.is_admin == False:
                ...
                if len(list(MyUser.objects.filter(is_admin=True))) == 1:
                    return messages.error(request, "ostatni administrator nie może zostać usunięty")
                else:
                    obj.delete()
...
class CustomDeleteAdmin(admin.ModelAdmin):
